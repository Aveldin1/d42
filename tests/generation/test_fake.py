...
def test_fake_unique_list():
    ...
    #     schema.str('test2') |
    #     schema.str('test')
    # ).len(0, ...).unique() Result: [], ['test', 'test3', 'test2', 'test4']
    # sch = schema.list(schema.int.min(1).max(4)).len(5).unique() Result: [2, 3, 1, 4]
    ...
    # sch = schema.list([schema.int(1), schema.int(1)]).unique() Result: [1]
    # sch = schema.list(schema.int(5)).unique()
    # sch = schema.list(schema.int(5)).len(3).unique()
    # sch = schema.list(schema.bool).len(3).unique()
    with given:
